...
from .llama3Tokenizer import Tokenizer as Llama31Tokenizer
...
def build_tokenizer(args):
    ...
    if args.tokenizer_type == 'BertWordPieceLowerCase':
        ...
        #print("Using GPTSentencePieceTokenizer")
        assert args.tokenizer_model is not None
        ...
        assert args.tokenizer_model is not None
        tokenizer = _Llama31Tokenizer(args.tokenizer_model)
...
# TH: Insert llama3 tokenizer as megatron subclass
class _Llama31Tokenizer(MegatronTokenizer): 
    def __init__(self, tokenizer_path): 
        super().__init__(tokenizer_path)
        self.tokenizer = Llama31Tokenizer(os.path.join(tokenizer_path, "tokenizer.model"))
        self.tokenizer.encoder = self.tokenizer.model._special_tokens | self.tokenizer.model._mergeable_ranks 
    ...
    @property
    def inv_vocab(self): 
        return {value: key for key, value in self.tokenizer.encoder.items()}
...
def create_llama3_tokenizer(*args, **kwargs):
    ...
    try:
        #from llama.tokenizer import Tokenizer as Llama3Tokenizer  # TH: Modified to use local version of tokenizer
        from .llama3Tokenizer import Tokenizer as Llama3Tokenizer
